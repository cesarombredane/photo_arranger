...
function validateActiveCrop() { if (!activePhoto.value) return; activePhoto.value.cropValidated = true; if (state.activeIndex < state.photos.length - 1) state.activeIndex++ }
function requestArrange() { if (unvalidatedCount.value) showArrangeWarning.value = true; else step.value = 3 }
function reviewUnvalidated() { const index = state.photos.findIndex(photo => !photo.cropValidated); if (index >= 0) state.activeIndex = index; showArrangeWarning.value = false }
...
<template>
  <q-layout view="hHh lpR fFf" class="bg-dark text-white"><AppHeader /><q-page-container><q-page class="q-pa-md q-pa-lg-xl">
    ...
    <q-dialog v-model="showLayoutEditor" persistent maximized transition-show="slide-up" transition-hide="slide-down"><q-card class="bg-dark text-white"><q-toolbar class="bg-grey-10"><q-icon name="crop" color="primary" size="28px" class="q-mr-md" /><q-toolbar-title>Edit photo</q-toolbar-title><q-btn flat round dense icon="close" @click="showLayoutEditor=false" /></q-toolbar><q-card-section v-if="activePhoto" class="row q-col-gutter-xl q-pa-lg"><div class="col-12 col-md-9"><q-card flat bordered class="bg-grey-10 text-white"><q-card-section><PhotoCropper /></q-card-section></q-card></div><div class="col-12 col-md-3"><q-card flat bordered class="bg-grey-10 text-white"><q-card-section><div class="text-overline text-primary">Photo size</div><div class="text-subtitle1 ellipsis q-mb-md">{{ activePhoto.name }}</div><q-option-group :model-value="activePhoto.templateId" :options="state.templates.map(t => ({ label: t.name, value: t.id }))" color="primary" type="radio" @update:model-value="setActiveTemplate" /></q-card-section><q-separator dark /><q-card-actions vertical><q-btn class="full-width" color="positive" icon="check_circle" label="Validate and close" @click="validateLayoutEdit" /><q-btn flat class="full-width" color="grey-4" label="Close" @click="showLayoutEditor=false" /></q-card-actions></q-card></div></q-card-section></q-card></q-dialog>
